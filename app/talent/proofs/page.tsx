...
import { ArrowLeft, UserCircle, Wallet, ShieldCheck, ChevronRight, AlertTriangle, Fingerprint, Zap, Lock, Loader2, LogOut, Copy, Check, LayoutDashboard, Award } from 'lucide-react';
    ...
    <div className="flex flex-col min-h-screen bg-black font-sans text-white">
      <header className="px-10 py-6 flex justify-between items-center border-b border-zinc-800 bg-black/50 backdrop-blur-2xl sticky top-0 z-50">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-3">
             ...
             <span className="text-2xl font-black tracking-tighter uppercase italic">Proof Factory</span>
          </div>

          <nav className="hidden md:flex items-center gap-4">
            <Link href="/talent/dashboard" className="flex items-center gap-2 px-4 py-2 text-zinc-400 hover:text-white transition-colors text-xs font-black uppercase tracking-widest">
              <LayoutDashboard className="w-4 h-4" />
              Dashboard
            </Link>
            <Link href="/talent/profile" className="flex items-center gap-2 px-4 py-2 text-zinc-400 hover:text-white transition-colors text-xs font-black uppercase tracking-widest">
      ...
      <footer className="px-10 py-8 border-t border-zinc-800 text-center flex flex-col md:flex-row items-center justify-between">
        ...
        <div className="mt-4 md:mt-0 text-[11px] font-black text-zinc-400 uppercase tracking-widest flex items-center gap-2">
           Proof Type: <span className="text-indigo-600 bg-indigo-900/20 px-3 py-1 rounded-lg italic">Groth16 / Midnight Compact</span>
        </div>
        <div className="mt-4 md:mt-0 text-zinc-400 text-[10px] font-black uppercase tracking-widest">
          Circuit execution happens locally in-browser
        </div>
      </footer>
    </div>
  );
