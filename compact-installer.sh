...
if [ -n "${COMPACT_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$COMPACT_INSTALLER_GHE_BASE_URL"
else
    INSTALLER_BASE_URL="${COMPACT_INSTALLER_GITHUB_BASE_URL:-https://github.com}"
fi
if [ -n "${COMPACT_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URL="$COMPACT_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URL="$INSTALLER_DOWNLOAD_URL"
else
    ARTIFACT_DOWNLOAD_URL="${INSTALLER_BASE_URL}/midnightntwrk/compact/releases/download/compact-v0.5.1"
fi
if [ -n "${COMPACT_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$COMPACT_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${COMPACT_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$COMPACT_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${COMPACT_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$COMPACT_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${COMPACT_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${COMPACT_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${COMPACT_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.30.3"},"source":{"app_name":"compact","name":"compact","owner":"midnightntwrk","release_type":"github"},"version":"0.5.1"}
EORECEIPT
...
RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/compact"
...
The installer for compact 0.5.1

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/midnightntwrk/compact/releases/download/compact-v0.5.1
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set COMPACT_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in
        "compact-aarch64-apple-darwin.tar.xz")
            ...
            _checksum_value="4d90647e4e8de0ec8d227eb6b01ab1c58223512e62bcf36b6a934ceef8179a08"
            _bins="compact"
            _bins_js_array='"compact"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
        ...
        "compact-aarch64-unknown-linux-musl.tar.xz")
            ...
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="bbeb53b34c895aa52e13a6375cbfe90bd670b2e6a72ba23356d383664a9536cf"
            _bins="compact"
            _bins_js_array='"compact"'
            _libs=""
            _libs_js_array=""
        ...
        "compact-x86_64-apple-darwin.tar.xz")
            ...
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="77b23c7bdf7c32f0cb2621bc03620e46c00842ef0010f2a9bbcaeca2a5d08a66"
            _bins="compact"
            _bins_js_array='"compact"'
            _libs=""
            _libs_js_array=""
        ...
        "compact-x86_64-unknown-linux-musl.tar.xz")
            ...
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="684c6b3d2eef9484aabba7a0820c166ae5c169f3aecf28cbea2074840263ba66"
            _bins="compact"
            _bins_js_array='"compact"'
            _libs=""
            _libs_js_array=""
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in
        "aarch64-apple-darwin")
            _archive="compact-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="compact-x86_64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="compact-aarch64-unknown-linux-musl.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "x86_64-apple-darwin")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="compact-x86_64-unknown-linux-musl.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${COMPACT_INSTALL_DIR:-}" ]; then
        _force_install_dir="$COMPACT_INSTALL_DIR"
